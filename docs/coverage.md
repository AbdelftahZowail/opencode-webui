...
| permissions/forms/questions | `GET /api/permission/request`, `GET /api/permission/saved`, `GET/POST /api/session/{id}/permission*`, `GET /api/form/request`, `GET/POST /api/session/{id}/form*`, legacy `question` routes | `pendingPermissions`, `replyPermission`, `pendingForms`, `formState/replyForm/cancelForm`, `question*` | `PendingRequestsModal` (single FIFO popup across all sessions), permission/form/question queue in store |
| inbox/steering | `GET /api/session/{id}/inbox`, `DELETE inbox/{id}`, `POST steer/queue` | `inboxList/Queue/Steer/Delete`, `inboxPrompt` | `InboxPanel`, composer queue-while-running |
...
| interrupt/background | `POST /api/session/{id}/interrupt?continue`, `POST background` | `interrupt`, `sessionBackground` | Esc two-step interrupt, Ctrl+B background subagents |
