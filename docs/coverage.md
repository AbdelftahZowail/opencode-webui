...
| mcp | `GET/PUT/DELETE /api/mcp*`, `POST connect/disconnect`, `GET resource` (+ `location` query on all) | `mcpList/Put/Delete/Connect/Disconnect/Resource` | `McpIndicator` header popover (per-server status, connect/disconnect). MCP add/remove (`mcpPut`/`mcpDelete`) and `mcpResource` are client-only. **Wave 0 fix:** `mcpPut` sent **PATCH** where the route is **PUT**, and every `/api/mcp/*` call dropped the `location` query — both corrected against the snapshot |
...
| plugin updates | `POST /api/plugin/check`, `POST /api/plugin/update`, `POST /api/plugin/await-activation` | `pluginCheck`, `pluginUpdate`, `pluginAwaitActivation` | (client wrapped, UI pending — roadmap P4). **No enable/disable route exists** upstream: there is nothing to toggle, so the UI must not offer one |
